...
inline double y_function(double x, double b1, double b2, double b3, double b4, double b5 ){
  if ( x == 0 ){return b1;}
    ...
    return b1*exp(-b2*x*x)+ b3*((1 - exp(-b4*x*x))/x)*exp(-b5*x);
  }
  else if (x < 0) {
    return b1*exp(-b2*x*x)+ b3*((1 - exp(-b4*x*x))/(-x))*exp(b5*x);
  }
...
inline double dy3_function(double x, double b1, double b2, double b3, double b4, double b5 ){
  if ( x == 0 ){
    ...
    return exp(-b4*x)*(1 - exp(-b5*x*x))/x;
  }
  else  if (x < 0){
    return exp(b4*x)*(1 - exp(-b5*x*x))/(-x);
  }
...
inline double dy5_function(double x, double b1, double b2, double b3, double b4, double b5 ){
  if ( x == 0 ){
    ...
    return -b3*exp(-b5*x)*(1 - exp(-b4*x*x));
  }
  else if(x < 0){
    return -b3*exp(-b5*x)*(1 - exp(-b4*x*x));
  }
...
//---------------------fit function---------------------------------------//
/*
void Fuc1g1y3D(double& x,double a[],double& yfit,double dyda[],int& ma)
{
...
triple *xyz_1g1y3D	   = NULL;
triple *origins_1g1y3D	   = NULL;
const int Norigins_1g1y3D = 3*3*3;

void Func1g1y3D(double x,double y,double z, double a[],double& yfit, double dyda[],int& ma)
{
...
inline void Func1g1y3D(const triple& x,const triple& x0,
		 double	 a[],
		 double& yfit,
		 double	 dyda[],
		 int&	 ma)
...
void Func1g1y3D(double& index,double a[],double& yfit,double dyda[],int& ma)
{
  int n = ((int*)&index)[0];
  yfit = 0.0;
  for(int i = 0; i < ma; i++) dyda[i]=0.0;
  for(int i = 0; i < Norigins_1g1y3D; i++){
    Func1g1y3D(xyz_1g1y3D[n], origins_1g1y3D[i], a,yfit,dyda,ma);
  }
  
}

void  Fit1g1y3D::fit(double datain[], double datain_sigma[], double* b, double& chisq)
{
...
#define datain_sigma(ix,iy,iz)	datain_sigma[ix+Nx*(iy+Ny*(iz))]
  ...
  xyz_1g1y3D		   = new triple[fit_lengh*fit_lengh*fit_lengh]();
  int ixyz = 0;
  ...
  origins_1g1y3D = new triple[Norigins_1g1y3D];
  
  {
	...
	for(double z = 0; z <1*Lz; z+=Lz){
	  origins_1g1y3D[count] = triple(x,y,z);
	  count++;
	}
      }
    }
  ...
  // The initial call
  alambda = -1.0;
  mrqmin_(x,y,dy, ndata,a,ia, ma,	covar,alpha, nca,chisq, Func1g1y3D, alambda);
