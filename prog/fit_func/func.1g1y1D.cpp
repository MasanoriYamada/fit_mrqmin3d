...
//---------------------fit for 3D---------------------------------------//
void Func1g1y1D(double& x,double a[],double& yfit,double dyda[],int& ma)
{
  double        b1 = a[0];
  double        b2 = a[1];
  double        b3 = a[2];
  ...
  if ( x == 0 ){yfit = b1;}
    ...
    yfit = b1*exp(-b2*x*x)+ b3*((1 - exp(-b4*x*x))/x)*exp(-b5*x);
  }
  ...
  if ( x == 0 ){
    ...
    dyda[2] = exp(-b4*x)*(1 - exp(-b5*x*x))/x;
    dyda[3] = b3*x*exp(-b5*x-b4*x*x);
    dyda[4] = -b3*exp(-b5*x)*(1 - exp(-b4*x*x));
  }
  else {cout<<"ERR:: fit data x is warng"<<endl;}
}

void  Fit1g1y1D::fit(double* datain, double* datain_sigma, double* b, double& chisq)
{
...
#define datain_sigma(ix,iy,iz)  datain_sigma[ix+Nx*(iy+Ny*(iz))]
  ...
  // The initial call
  alambda = -1.0;
  mrqmin_(x,y,dy, ndata,a,ia, ma,	covar,alpha, nca,chisq, Func1g1y1D, alambda);
